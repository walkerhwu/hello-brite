describe('PokeAPI Tests', () => {
    ...
    const berryBaseUrl = `${baseUrl}/berry`;
    ...
    before(() => {
        cy.request({
            ...
            expect(response.body.results).not.to.be.empty;
    ...
    describe('Berry by ID', () => {
        it('should return berry data for valid ID', () => {
            cy.request({
                method: 'GET',
                url: `${berryBaseUrl}/${validBerryId}/`,
            }).then((response) => {
                expect(response.status).to.equal(200);
                expect(response.body).to.have.property('id', parseInt(validBerryId));
    ...
    describe('Berry by Name', () => {
        it('should return berry data for valid name', () => {
            cy.request({
                method: 'GET',
                url: `${berryBaseUrl}/${validBerryName}/`,
            }).then((response) => {
                expect(response.status).to.equal(200);
